 ...
 * Uses smart detection: checks migration history AND actual database state
...
async function isMigrationApplied(version) {
  ...
  try {
    ...
    const migrationRecord = result[0] || result.rows?.[0];

    // If completed, definitely applied
    if (migrationRecord?.status === 'completed') {
      return true;
    }

    // If no record, definitely not applied
    if (!migrationRecord) {
      return false;
    }

    // If failed, check if the database objects actually exist (smart detection)
    if (migrationRecord?.status === 'failed') {
      const actuallyApplied = await checkMigrationObjectsExist(version);

      if (actuallyApplied) {
        // Migration failed but objects exist - mark as completed
        console.log({
          event: 'migration_smart_recovery',
          version,
          instance_id: INSTANCE_ID,
          message: 'Failed migration but objects exist, marking completed'
        });

        await recordMigrationAttempt(version, 'completed', {
          checksum: 'recovered',
          executionTimeMs: 0
        });
    ...
    // Pending or failed without objects - not applied
    return false;
  } catch (error) {
...
/**
 * Check if the database objects for a migration actually exist
 * @param {string} version - Migration version
 * @returns {Promise<boolean>}
 */
async function checkMigrationObjectsExist(version) {
  const db = getConnection();
  ...
  try {
      ...
      case '001_initial.sql':
        ...
        const tablesResult = await db`
          ...
          WHERE table_schema = 'public'
            AND table_name IN ('query_logs', 'eval_results')
        `;
        const tableCount = parseInt((tablesResult[0] || tablesResult.rows?.[0])?.count || 0);
        return tableCount >= 2;

      case '002_routing_decisions.sql':
        // Check if routing tables exist
        const routingResult = await db`
          SELECT COUNT(*) as count
          FROM information_schema.tables
          WHERE table_schema = 'public'
            AND table_name IN ('routing_decisions', 'retrieval_details')
        `;
        const routingCount = parseInt((routingResult[0] || routingResult.rows?.[0])?.count || 0);
        return routingCount >= 2;

      case '003_analytics_enhancements.sql':
        ...
        const columnsResult = await db`
          ...
          WHERE table_schema = 'public'
            ...
            AND column_name IN ('embedding_tokens', 'estimated_cost')
        `;
        const columnCount = parseInt((columnsResult[0] || columnsResult.rows?.[0])?.count || 0);
        return columnCount >= 2;

      case '004_migration_history.sql':
        // Check if migration history tables exist
        const historyResult = await db`
          SELECT COUNT(*) as count
          FROM information_schema.tables
          WHERE table_schema = 'public'
            AND table_name IN ('migration_history', 'migration_locks')
        `;
        const historyCount = parseInt((historyResult[0] || historyResult.rows?.[0])?.count || 0);
    ...
    console.error(`Error checking objects for migration ${version}:`, error);
    return false;
  }
}
