...
export async function getQueryStats(hours = 24) {
  ...
  return executeWithRetry(async () => {
    // Use PostgreSQL INTERVAL syntax that works in Vercel Postgres
    const result = await db`
      SELECT
        routing_layer,
        COUNT(*) as layer_count,
        COUNT(CASE WHEN error_message IS NOT NULL THEN 1 END) as layer_errors,
    ...
    for (const row of rows) {
      if (row.routing_layer) {
        const count = parseInt(row.layer_count) || 0;
        const errors = parseInt(row.layer_errors) || 0;
