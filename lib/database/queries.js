...
export async function getQueryStats(hours = 24) {
  ...
  return executeWithRetry(async () => {
    ...
    for (const row of result) {
      stats.totalQueries = parseInt(row.total_queries);
      stats.errorCount = parseInt(row.error_count);
      stats.avgResponseTime = parseFloat(row.avg_response_time);
      stats.p95ResponseTime = parseFloat(row.p95_response_time);
      stats.routingBreakdown[row.routing_layer] = parseInt(row.count);
    }
