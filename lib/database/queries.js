  ...
  return executeWithRetry(async () => {
    ...
    const overallStatsResult = await db`
      SELECT
        ...
        COUNT(DISTINCT routing_layer) as routing_layers_used
      FROM query_logs
      WHERE timestamp >= NOW() - INTERVAL '1 hour' * ${hours}
        AND environment = ${getCurrentEnvironment()}
    `;

    // Get routing breakdown separately
    const routingBreakdownResult = await db`
      SELECT
        routing_layer,
        COUNT(*) as count,
        AVG(response_time_ms) as avg_response_time
    ...
    // Handle both array-style and rows-style results from Vercel Postgres
    const overallRows = overallStatsResult.rows || overallStatsResult;
    const routingRows = routingBreakdownResult.rows || routingBreakdownResult;

    // Process overall stats
    const overallRow = overallRows[0] || {};
    const stats = {
      totalQueries: parseInt(overallRow.total_queries) || 0,
      errorCount: parseInt(overallRow.error_count) || 0,
      avgResponseTime: parseFloat(overallRow.avg_response_time) || 0,
      p95ResponseTime: parseFloat(overallRow.p95_response_time) || 0,
      routingLayersUsed: parseInt(overallRow.routing_layers_used) || 0,
      routingBreakdown: {}
    };

    // Process routing breakdown
    for (const row of routingRows) {
      if (row.routing_layer) {
        stats.routingBreakdown[row.routing_layer] = {
