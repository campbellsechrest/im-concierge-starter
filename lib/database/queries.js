  ...
  return executeWithRetry(async () => {
    const result = await db`
      INSERT INTO query_logs (
        ...
        ${queryData.embeddingTokens || null},
        ${queryData.chatCompletionTokens || null},
        ${queryData.estimatedCost || null},
        ${queryData.apiCallsCount || null}
      )
      RETURNING id
    `;
...
export async function logRetrievalDetails(queryLogId, retrievalDetails) {
  ...
  if (!queryLogId) {
    throw new Error('queryLogId is required for logRetrievalDetails');
  }
  ...
  return executeWithRetry(async () => {
    ...
    for (let index = 0; index < retrievalDetails.length; index++) {
      ...
      if (!detail.documentId) {
        console.warn(`Skipping retrieval detail ${index}: missing documentId`, detail);
        continue;
      }

      if (typeof detail.similarityScore !== 'number') {
        console.warn(`Skipping retrieval detail ${index}: invalid similarityScore`, detail);
        continue;
      }
      ...
      try {
        await db`
          INSERT INTO retrieval_details (
            ...
            ${index + 1},
            ${Boolean(detail.scopeFiltered)}
          )
        `;
      } catch (error) {
        console.error(`Failed to insert retrieval detail ${index} for query ${queryLogId}:`, error.message, detail);
...
export async function logRoutingDecisions(queryLogId, decisionTrace) {
  ...
  // Validate required fields
  if (!queryLogId) {
    throw new Error('queryLogId is required for logRoutingDecisions');
  }
  ...
  return executeWithRetry(async () => {
    ...
    for (let index = 0; index < decisionTrace.length; index++) {
      ...
      if (!decision.layer) {
        console.warn(`Skipping routing decision ${index}: missing layer`, decision);
        continue;
      }

      if (typeof decision.triggered !== 'boolean') {
        console.warn(`Skipping routing decision ${index}: triggered must be boolean, got ${typeof decision.triggered}`, decision);
        continue;
      }
      ...
      try {
        await db`
          INSERT INTO routing_decisions (
            ...
            ${decision.score || null},
            ${Boolean(decision.triggered)},
            ${index + 1},
            ${decision.riskTokenCount || null},
            ${decision.hasProductContext || null},
            ${decision.embeddingScore || null},
            ${decision.executionTime || null},
            ${decision.apiLatency || null}
          )
        `;
      } catch (error) {
        console.error(`Failed to insert routing decision ${index} for query ${queryLogId}:`, error.message, decision);
        // Continue with other inserts rather than failing completely
      }
    }
  });
